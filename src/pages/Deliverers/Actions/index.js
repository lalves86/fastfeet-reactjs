...
export default function Actions(props) {
  ...
  const [deliverer] = useState(props.data);
  ...
  async function handleDelete() {
    await api.delete(`/deliverers/${deliverer.id}`);
  ...
  return (
    <Container>
      ...
      {visible && (
        <ActionList>
          <UpdateOrder
            onClick={() => history.push(`newdeliverer/${deliverer.id}`)}
          >
          ...
          >
            <ConfirmExclusion>
              <h4>ATENÇÃO!!!</h4>
              <h5>Isto irá excluir o entregador permanentemente!</h5>
